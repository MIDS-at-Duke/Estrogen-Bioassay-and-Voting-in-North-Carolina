...
```{r}
partial_estrogen_model1 <- lmer(log(uterus)~ mweight + EE + ZM + (1 | lab), data = estrogen)
...
full_estrogen_model <- lmer(log(uterus)~ mweight + EE + ZM + protocol + (1 | lab) + EE:protocol + ZM:protocol, data = estrogen)
#tested intearction terms, no change to QQ plot
#intearction terms do not increase or decrease model AIC by much. Exhausted interaction terms for QQ list
#did not do factors for EE and ZM because that would result in loss of information since 1 mg is not just different from 10 mg, it is 1/10 of 10 milligrams.
...
logxe_estrogen_model <- lmer(log(uterus)~ mweight + EE + ZM + protocol + (1 | lab), data = estrogen)
...
AIC(partial_estrogen_model2)
AIC(full_estrogen_model)
BIC(partial_estrogen_model2)
BIC(full_estrogen_model)
#The full model is 
```
...
plot(partial_estrogen_model2)
vif(partial_estrogen_model2)
resid2 <- resid(partial_estrogen_model2)
plot(resid2)
#Not 
```
...
partial_estrogen_model3 <- lmer(log(uterus)~ EE + ZM + protocol + (1 + protocol| lab), data = estrogen)
summary(partial_estrogen_model3)
ranef(partial_estrogen_model3)
...
plot(logx_estrogen_model)
#log weight does not lead to correction of 
...
qqnorm(residuals(logx_estrogen_model));qqline(residuals(logx_estrogen_model))
qqnorm(residuals(partial_estrogen_model3));qqline(residuals(partial_estrogen_model3))
qqnorm(residuals(full_estrogen_model));qqline(residuals(full_estrogen_model))
# not very good 
